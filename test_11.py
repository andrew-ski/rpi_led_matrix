...
#    color_background = graphics.Color(0, c_bg_g, c_bg_b)
...
#    graphics.DrawLine(offscreen_canvas, 1, 14, 3, 14, graphics.Color(255,0,0))
 #   if frame == 1:
 #     f1x = randint(0,4)
 #     graphics.DrawLine(offscreen_canvas,f1x, 9, f1x, 9, graphics.Color(200,160,0))
 #     frame = randint(1,3)
 #   elif frame == 2:
...
#    graphics.DrawLine(offscreen_canvas,0, 14, 2, 12, graphics.Color(200,60,0))
    ...
    f_r_r = randint(100, 255)
    f_r_g = f_r_r/25.5
    f_red = graphics.Color(f_r_r, f_r_g, 0)

    f_o_r = randint(100,255)
    f_o_g = f_r_r/4.25 
    f_orange = graphics.Color(f_o_r, f_o_g, 0)

    f_y_r = randint(100,200)
    f_y_g = f_r_r/1.3
    f_yellow = graphics.Color(f_y_r, f_y_g, 0)

    graphics.DrawLine(offscreen_canvas,2, 10, 2, 10, f_red)
    graphics.DrawLine(offscreen_canvas,1, 11, 1, 11, f_red)
    graphics.DrawLine(offscreen_canvas,0, 13, 0, 13, f_red)

    graphics.DrawLine(offscreen_canvas,3, 12, 3, 12, f_orange)
    graphics.DrawLine(offscreen_canvas,2, 11, 2, 11, f_orange)
    graphics.DrawLine(offscreen_canvas,1, 13, 1, 12, f_orange)
    graphics.DrawLine(offscreen_canvas,0, 14, 0, 14, f_orange)

    graphics.DrawLine(offscreen_canvas,1, 15, 3, 15, f_yellow)
    graphics.DrawLine(offscreen_canvas,2, 12, 2, 12, f_yellow)
    graphics.DrawLine(offscreen_canvas,2, 13, 3, 13, f_yellow)
    graphics.DrawLine(offscreen_canvas,1, 14, 4, 14, f_yellow)
    graphics.DrawLine(offscreen_canvas,1, 15, 3, 15, f_yellow)
